...
- 룬 해제(미니맵 방식): **미니맵의 보라색 룬 표식에 노란색 캐릭터 표식을 겹치도록 이동** →
  활성화 키(SPACE 등) → 화살표 판독 → 방향키 입력 → 해제 확인.
  룬이 화면 밖에 있어도 미니맵으로 찾아갑니다
- 룬 해제(템플릿 방식): 화면에 보이는 룬 이미지를 찾아 접근 (좁은 맵용, 선택)
...
📖 **자세한 사용법은 [docs/USAGE.md](docs/USAGE.md) 를 보세요** — 설치부터 템플릿 만들기, 항목별 권장값,
로그 읽는 법, 증상별 해결까지 순서대로 정리해 두었습니다.
...
| 단계 | 하는 일 | 관련 설정 |
| --- | --- | --- |
| 감지 | 미니맵에 보라색 룬 표식이 보이면(또는 화면에서 룬 템플릿을 찾으면) 즉시 모든 스킬 입력 중단 | 룬 찾는 방식, 미니맵 영역·색, 룬 탐색 주기 |
| 접근 | 미니맵의 dx/dy 를 보고 방향키로 좌우 정렬, 룬이 위면 로프 커넥트(A), 아래면 아래+점프(ALT). 미니맵 배율은 실제 이동량으로 자동 보정 | 좌우/높이 허용 오차, 이동 환산 계수, 자동 보정, 정렬 제한 시간 |
| 활성화 | 정렬 완료 후 활성화 키(SPACE 등) 입력, 화살표 UI 가 뜨는지 확인 | 룬 활성화 키, 활성화 시도 횟수 |
...
개발 VM(합성 1024x768 화면) 기준 측정값입니다. `python -m rune_hunter.tools.bench` 로 직접 측정할 수 있고,
`--live` 옵션을 주면 실제 게임 창을 캡처해서 측정합니다.

| 항목 | 1회 소요 |
| --- | --- |
| 룬 감지 (미니맵 색상, 기본 방식) | **평균 0.6 ms** |
| 룬 감지 (전체 화면 템플릿) | 평균 8.0 ms |
| 룬 감지 (탐색 영역 축소) | 평균 3.1 ms |
| 룬 감지 (감지 축소 0.5배) | 평균 2.1 ms |
| 화살표 4개 판독 | 평균 5.8 ms |
| 대기 정확도 (5 / 20 / 120 ms 목표) | 오차 +0.14 ms (최대 +0.33 ms) |

데모 세계에서 100초 연속 실행한 결과 (사냥키 0.1초, 버프 3개, 룬 탐색 0.5초, 룬 8회 등장):

```
룬: 등장 8회 / 해제 8회 / 오입력 0회
키 입력: 사냥기 818회, 버프 각 5회
제어 루프: 132회/초   루프 지연: 평균 0.11ms, 최대 4.79ms
감지: 123회, 평균 9.7ms   CPU 5.8%   메모리 88MB (증가 없음)
```

데모 모드는 매 프레임을 합성해서 그리기 때문에 CPU 5.8% 중 상당 부분이 그 비용입니다.
실제 게임에서는 화면 캡처(mss, 창 영역만)가 훨씬 저렴합니다.
...
- Windows 에서는 `timeBeginPeriod(1)` 로 타이머 해상도를 1ms 로 올립니다. 4ms 이상 대기는
  일반 `sleep` 을 쓰고, 1ms 미만의 짧은 대기만 스핀으로 채웁니다. (모든 대기의 마지막 1.5ms 를
  스핀으로 채우던 초기 구현은 4ms 주기 루프에서 CPU 를 35% 나 썼습니다 → 현재 0.6%)
...
| `tests/test_minimap.py` | 미니맵 표식 좌표·방향 인식, 화면 밖 룬 감지, 색 추출, 정렬 후 SPACE 해제, 표식 소실/색 오설정 시 처리 |
...
| `tests/test_timing.py` | 대기 정확도, 대기 중 CPU 를 태우지 않음(스핀 회귀 방지) |
| `tests/test_config.py` | 프로필 저장·불러오기, 일부 필드만 있는 예전 프로필 호환 |
| `tests/test_capture_tools.py` | 영역 선택 좌표 환산, 잘라낸 템플릿으로 원본 재검출, 한글 경로 저장 |
...
| 창이 뜨자마자 닫힘 | `run.bat` 이 콘솔에 오류를 남기고 멈춥니다. `.venv\Scripts\python -m rune_hunter --diagnose` 로 점검, `logs\crash.log` 확인 |
